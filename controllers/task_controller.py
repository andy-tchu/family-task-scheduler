...
def update_task(id, task):
    ...
    try:
        db_conn = get_db_conn()
        db_conn["tasks"].update_one({"_id": ObjectId(id)},{"$set": task}, upsert=False)
        logging.info(f"Updated a task with a task_id of: {id}")
        return {"message": "Updated task", "id": id}
...
def delete_task(id):
    try:
        db_conn = get_db_conn()
        db_conn["tasks"].delete_one({"_id": ObjectId(id)})
        logging.info(f"Deleted a task with a task_id of: {id}")
        return {"message": "Deleted task", "id": id}
