...
def get_all_tasks():
    try:
        ...
        tasks = list(db_conn['tasks'].find())
        logging.info(f"Looked and found tasks: {len(tasks)}")
...
def get_all_family_tasks(id):
    try:
        ...
        tasks = list(db_conn['tasks'].find({"familyId": found_family["_id"]}))
        logging.info(f"Looked and found family tasks: {len(tasks)}")
        return json.loads(json_util.dumps(tasks))
    except Exception as e:
        logging.error(str(e))
        abort(500)
