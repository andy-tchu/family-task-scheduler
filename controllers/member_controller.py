...
def update_member(id, member):
    ...
    try:
        db_conn = get_db_conn()
        db_conn["members"].update_one({"_id": ObjectId(id)},{"$set": member}, upsert=False)
        logging.info(f"Updated a member with a member_id of: {id}")
        return {"message": "Updated member", "id": id}
...
def delete_member(id):
    try:
        db_conn = get_db_conn()
        db_conn["members"].delete_one({"_id": ObjectId(id)})
        logging.info(f"Deleted a member with a member_id of: {id}")
        return {"message": "Deleted member", "id": id}
